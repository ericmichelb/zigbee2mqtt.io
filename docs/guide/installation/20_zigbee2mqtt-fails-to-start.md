...
5. The mqtt server definition in Zigbee2MQTT is not pointing to primary IP of HA. Use "ha network info" to retrieve the IP that holds: <primary: true>. In Zigbee2MQTT > mttq: Change from server: 'mqtt://localhost:1883'  to  server: 'mqtt://<IP.eth0>:1883'. Restart the add-on. 
